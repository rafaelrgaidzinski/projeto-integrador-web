...
import { mockDataPurchase } from "../../data/mockData";
...
function CustomFooter(props) {
    ...
    const total = props.mockDataPurchase.reduce((sum, row) => sum + row.total, 0);
...
const Purchase = () => {
    ...
    return (
        <Box m={"10px 20px 0 20px"}>
            <Header title={"Cadastro de Compras"}/>
            <Box m={"10px 0 0 0"} height={"40vh"} sx={{
                ...
                <DataGrid rows={mockDataPurchase} columns={columns}  slots={{footer: CustomFooter}} slotProps={{footer: { mockDataPurchase, colors }}}/>
            </Box>
            ...
            <PaymentDialog open={openPayment} handleClickOpen={handleClickOpenPayment} handleClose={handleClosePayment} page={"fornecedores"}/>
        </Box>
    )
}
